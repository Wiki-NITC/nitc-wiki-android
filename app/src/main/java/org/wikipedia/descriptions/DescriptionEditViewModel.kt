...
class DescriptionEditViewModel(savedStateHandle: SavedStateHandle) : ViewModel() {
    ...
    fun loadPageSummary() {
        viewModelScope.launch(CoroutineExceptionHandler { _, throwable ->
            ...
            val summaryResponse = async { ServiceFactory.getPageSummary(pageTitle.wikiSite,pageTitle.prefixedText) }
    ...
    fun waitForRevisionUpdate(newRevision: Long) {
        viewModelScope.launch(CoroutineExceptionHandler { _, throwable ->
            ...
            while (revision < newRevision && retry < 10) {
                delay(2000)
                val pageSummary = ServiceFactory.getPageSummary(pageTitle.wikiSite,pageTitle.prefixedText)
